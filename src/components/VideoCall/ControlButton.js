...
export default function ControlButton({ handleClick, isActive, textActive, textUnActive, iconPath }) {
    ...
    useIsomorphicLayoutEffect(() => {
        if (isActive) {
            controlButtonRef.current.classList.add("bg-gray-700");
            controlTextRef.current.innerHTML = textActive;
        } else {
            controlButtonRef.current.classList.remove("bg-gray-700");
    ...
    return (
        <div className="mx-1">
            ...
            >
                ...
                <div
                    className="fixed text-sm mobileSE:text-xs mobile:text-sm text-white mt-[35px] w-[120px]"
                    ref={controlTextRef}
                ></div>
            </motion.button>
        </div>
