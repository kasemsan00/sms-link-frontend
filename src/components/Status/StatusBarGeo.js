...
const StatusbarGeo = ({ show }) => {
      ...
      navigator.geolocation.watchPosition(
        async (position) => {
          ...
          if (response) {
            ...
            locationNameRef.current.innerHTML = response.data.subdistrict + " " + response.data.district + " " + response.data.province;
            dispatch(
              setLocation({
                status: "Success",
  ...
  return (
    <>
      <Head>{show ? <meta name="theme-color" content="#3A80DB" /> : null}</Head>
      <div className="flex flex-1 items-center justify-center h-[24px] bg-[#3A80DB]" ref={statusBarRef}>
