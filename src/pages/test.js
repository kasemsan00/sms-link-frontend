...
export default function Test() {
  ...
  useEffect(() => {
    if (remoteVideoRef !== null && remoteVideoRef.current.srcObject === null && temporaryStream === null) {
      temporaryStream = new MediaStream();
      remoteVideoRef.current.srcObject = temporaryStream.remoteStream;
  ...
  const startRecord = () => {
    mediaRecorder = new MediaRecorder(remoteVideoRef.current.captureStream());
  ...
  const initCamera = async () => {
    await getCamera("user");
    await startRecord();
  };

  const changeCamera = async () => getCamera("environment");
  ...
  const getCamera = async (facingMode) => {
    ...
    remoteVideoRef.current.srcObject.replaceVideoTrack(stream.getVideoTracks()[0]);
    remoteVideoRef.current.srcObject.replaceAudioTrack(stream.getAudioTracks()[0]);
  };
  ...
  return (
    <div>
      <div className="w-full h-full fixed flex flex-1 flex-row justify-center items-center z-50 cursor-pointer ">
        <button className="w-[150px] mx-5 btn" onClick={() => initCamera()}>
          Init
        </button>
        <button className="w-[150px] mx-5 btn" onClick={() => changeCamera()}>
          ChangeCamera
        </button>
        <button className="w-[150px] mx-5 btn" onClick={() => stopRecord()}>
          StopRecord
        </button>
      </div>
      <video className="w-[200px] h-[200px]" autoPlay playsInline ref={remoteVideoRef}></video>
    </div>
  );
}
